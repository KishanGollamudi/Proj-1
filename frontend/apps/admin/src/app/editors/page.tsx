...
export default function AdminEditorsPage() {
  ...
  const { data, refetch } = useQuery({
    queryKey: ['admin-editors', applicationStatus],
    queryFn: () => apiGet<EditorItem[]>('/admin/editors', session?.user?.accessToken, { applicationStatus }),
    enabled: Boolean(session?.user?.accessToken)
  });

  async function review(id: string, status: 'APPROVED' | 'REJECTED') {
    await apiPost(`/admin/editors/${id}/review`, { status }, session?.user?.accessToken);
    showToast(`Editor ${status.toLowerCase()}`);
    await refetch();
  }

  async function feature(id: string, featured: boolean) {
    await apiPost(`/admin/editors/${id}/featured`, { featured }, session?.user?.accessToken);
    showToast(featured ? 'Editor featured' : 'Editor unfeatured');
    await refetch();
  }
        ...
        {(data ?? []).map((item) => (
          <div key={item.id} className="flex items-center justify-between rounded border bg-white p-3 text-sm">
            <div>
              ...
              <p className="text-slate-600">{item.email} • {item.editorProfile?.applicationStatus}</p>
            </div>
            <div className="flex gap-2">
              ...
              <button onClick={() => feature(item.id, !item.editorProfile?.isFeatured)} className="rounded border px-2 py-1">{item.editorProfile?.isFeatured ? 'Unfeature' : 'Feature'}</button>
            </div>
          </div>
        ))}
      </div>
